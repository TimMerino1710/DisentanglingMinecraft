...
class MinecraftVisualizerPyVista:
    ...
    def create_voxel_gif(self, voxel_sequence, output_path='voxel_animation.gif', duration=2.0, highlight_latents=None, show_axis=True, transparent_background=False, fps=2, wireframe_highlight=False, save_frames=False):
        """
        Create an animated visualization using PIL with proper frame disposal.
        
        Args:
            voxel_sequence: Sequence of voxel data to animate
            output_path: Path to save the output GIF
            duration: Duration of the animation in seconds
            highlight_latents: Optional latent coordinates to highlight
            show_axis: Whether to show the axis
            transparent_background: Whether to use a transparent background
            fps: Frames per second
            wireframe_highlight: Whether to use wireframe for highlights
            save_frames: If True, saves individual frames as PNG files in a directory
        """
        ...
        # Create frames directory if save_frames is True
        if save_frames:
            frames_dir = os.path.splitext(output_path)[0] + '_frames'
            os.makedirs(frames_dir, exist_ok=True)
        
        # Create frames
        for i, voxels in enumerate(voxel_sequence):
            plotter = pv.Plotter(off_screen=True)
            if highlight_latents is not None:
            ...
            # Save individual frame if save_frames is True
            if save_frames:
    ...
    def visualize_both_codes_with_blocks(self, style_latents, struct_latents, block_data, plotter=None):
        ...
        Args:
            style_latents: Tensor/array of shape [D,H,W] containing style latent codes
            struct_latents: Tensor/array of shape [D,H,W] containing structure latent codes
            block_data: Tensor/array containing block data
        ...
        latent_depth, latent_height, latent_width = style_latents.shape
        block_size = block_data.shape[0] if isinstance(block_data, np.ndarray) else block_data.shape[1]
        scale_factor = block_size // latent_width  # Calculate scale factor between latent and block space
        ...
        style_actors = {i: [] for i in range(latent_height)}
        struct_actors = {i: [] for i in range(latent_height)}
        style_texts = {}
        struct_texts = {}
        
        # Calculate offset for visualization spacing
        vis_spacing = block_size  # Add some padding between visualizations
        
        # Create all actors at once
        for layer in range(latent_height):
            style_centers = []
            struct_centers = []
            style_labels = []
            struct_labels = []
            ...
            for d in range(latent_depth):
                for w in range(latent_width):
                    ...
                    style_cube = pv.Cube(bounds=(
                        x * scale_factor + vis_spacing, (x + 1) * scale_factor + vis_spacing,
                        w * scale_factor, (w + 1) * scale_factor,
                        layer * scale_factor, (layer + 1) * scale_factor
                    ))
                    ...
                    struct_cube = pv.Cube(bounds=(
                        ...
                        w * scale_factor + vis_spacing, (w + 1) * scale_factor + vis_spacing,
                        layer * scale_factor, (layer + 1) * scale_factor
                    ))
        ...
        # Add static labels with adjusted positions
        for label, pos, rot in [
            ("Style codes", (vis_spacing * 2 + 8, block_size // 2, 0), 90),
            ("Structure codes", (block_size//2, vis_spacing * 2 + 8, 0), 180)
        ]:
